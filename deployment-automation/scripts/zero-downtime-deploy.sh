...
ACTIVE_PORT=$(grep -oE 'proxy_pass http://(localhost|127\.0\.0\.1):[0-9]+' "$NGINX_CONFIG" | tail -1 | grep -oE '[0-9]+$' || true)
OLD_BACKEND=""
...
NGINX_TEMP=$(mktemp)
sed -E "s#proxy_pass http://(localhost|127\.0\.0\.1):[0-9]+;#proxy_pass http://127.0.0.1:$NEW_PORT;#" "$NGINX_CONFIG" > "$NGINX_TEMP"
