...
export class PostLimitsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadServiceAreaConfig(): void {
    ...
    this.settingsService.getAllSettings().subscribe({
      ...
      error: () => {
        this.swal.toast('Failed to load service area config', 'error');
        this.serviceAreaLoading = false;
      }
    });
  }
  ...
  saveServiceAreaConfig(): void {
    ...
    this.settingsService.updateMultipleSettings(settings).subscribe({
      next: () => {
        this.swal.toast('Service area config saved successfully', 'success');
        this.serviceAreaSaving = false;
      },
      error: () => {
        this.swal.toast('Failed to save service area config', 'error');
        this.serviceAreaSaving = false;
      }
    });
  }
  ...
  loadGlobalFreePostLimit(): void {
    ...
    this.settingsService.getAllSettings().subscribe({
      ...
      error: () => {
        this.swal.toast('Failed to load global free post limit', 'error');
        this.globalFreePostLimitLoading = false;
      }
    });
  }
  ...
  saveGlobalFreePostLimit(): void {
    ...
    this.settingsService.updateMultipleSettings(settings).subscribe({
      next: () => {
        this.swal.toast('Global free post limit saved successfully', 'success');
        this.globalFreePostLimitSaving = false;
      },
      error: () => {
        this.swal.toast('Failed to save global free post limit', 'error');
        this.globalFreePostLimitSaving = false;
      }
    });
  }
  ...
  loadGlobalLimits(): void {
    ...
    this.featureConfigService.getAllFeatures().subscribe({
      ...
      error: () => {
        this.swal.toast('Failed to load global limits', 'error');
        this.globalLoading = false;
      }
    });
  }
  ...
  saveGlobalLimit(config: FeatureConfig): void {
    ...
    this.featureConfigService.updateFeature(config.id!, updated).subscribe({
      next: () => {
        this.swal.toast(`Global limit for ${this.getGlobalFeatureLabel(config.featureName)} updated`, 'success');
        this.editingGlobalId = null;
        this.loadGlobalLimits();
      },
      error: () => this.swal.toast('Failed to update global limit', 'error')
    });
  }
  ...
  loadLimits(): void {
    ...
    this.postLimitsService.getAllLimits().subscribe({
      ...
      error: () => {
        this.swal.toast('Failed to load user overrides', 'error');
        this.isLoading = false;
      }
    });
  }
  ...
  saveLimit(): void {
    ...
    this.postLimitsService.createOrUpdate({
      ...
      next: () => {
        this.swal.toast('User override saved successfully', 'success');
        this.showForm = false;
        this.lookupResult = null;
        this.lookupError = '';
        this.loadLimits();
  ...
  deleteLimit(limit: UserPostLimit): void {
    ...
    this.postLimitsService.deleteLimit(limit.id!).subscribe({
      next: () => {
        this.swal.toast('User override deleted', 'success');
        this.loadLimits();
      },
      error: () => this.swal.toast('Failed to delete override', 'error')
    });
  }
  ...
  loadPaidPostConfig(): void {
    ...
    this.settingsService.getAllSettings().subscribe({
      ...
      error: () => {
        this.swal.toast('Failed to load paid post config', 'error');
        this.paidPostLoading = false;
      }
    });
  }
  ...
  savePaidPostConfig(): void {
    ...
    this.settingsService.updateMultipleSettings(settings).subscribe({
      next: () => {
        this.swal.toast('Paid post config saved successfully', 'success');
        this.paidPostSaving = false;
      },
      error: () => {
        this.swal.toast('Failed to save paid post config', 'error');
        this.paidPostSaving = false;
      }
    });
  }
  ...
  loadPostTypeSettings(): void {
    ...
    this.settingsService.getAllSettings().subscribe({
      ...
      error: () => {
        this.swal.toast('Failed to load post type settings', 'error');
        this.typeSettingsLoading = false;
      }
    });
  }
  ...
  savePostTypePrices(): void {
    ...
    this.settingsService.updateMultipleSettings(settings).subscribe({
      next: () => {
        this.swal.toast('Post type prices saved successfully', 'success');
        this.savingPrices = false;
      },
      error: () => {
        this.swal.toast('Failed to save post type prices', 'error');
        this.savingPrices = false;
      }
    });
  }
  ...
  saveBannerConfig(): void {
    ...
    this.settingsService.updateMultipleSettings(settings).subscribe({
      next: () => this.swal.toast('Banner config saved', 'success'),
      error: () => this.swal.toast('Failed to save banner config', 'error')
    });
  }
  ...
  saveBannerPrices(): void {
    ...
    this.settingsService.updateMultipleSettings(settings).subscribe({
      next: () => {
        this.swal.toast('Banner prices saved successfully', 'success');
        this.savingBannerPrices = false;
      },
      error: () => {
        this.swal.toast('Failed to save banner prices', 'error');
        this.savingBannerPrices = false;
      }
    });
  }
