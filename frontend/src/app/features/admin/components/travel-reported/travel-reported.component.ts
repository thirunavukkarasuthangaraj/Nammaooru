...
@Component({
  selector: 'app-travel-reported',
  templateUrl: './travel-reported.component.html',
  styleUrls: ['./travel-reported.component.scss']
})
export class TravelReportedComponent implements OnInit {
  posts: TravelPost[] = [];
  loading = true;
  currentPage = 0;
  totalPages = 0;
  ...
  loadReportedPosts(): void {
    this.loading = true;
    this.travelService.getReportedPosts(this.currentPage, this.pageSize).subscribe({
      next: (response) => {
  ...
  onStatusChange(post: TravelPost, newStatus: string): void {
  ...
  getAvailableStatuses(post: TravelPost): StatusOption[] {
