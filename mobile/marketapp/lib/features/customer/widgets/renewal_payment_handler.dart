...
class RenewalPaymentHandler {
  ...
  Future<void> _startPayment({required int count}) async {
    ...
    // Gross-up formula: Razorpay charges 2% + 18% GST = 2.36% on the gross amount.
    // To break even, we calculate: gross = ceil(base / (1 - 0.0236)) + 1 (buffer for Razorpay rounding).
    // Example: ₹15 base → gross = ceil(1500/0.9764)+1 = 1538 paise → fee = 38 paise = ₹0.38
    final int singleProcessingFee = config['processingFeePaise'] ??
        ((pricePerPost * 100 / 0.9764).ceil() + 1 - pricePerPost * 100);
