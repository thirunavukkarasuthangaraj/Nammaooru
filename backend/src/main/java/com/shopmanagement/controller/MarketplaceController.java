...
public class MarketplaceController {
    ...
    public ResponseEntity<ApiResponse<MarketplacePost>> toggleFeatured(@PathVariable Long id) {
        try {
            MarketplacePost post = marketplaceService.toggleFeatured(id);
            ...
            log.error("Error toggling featured status for marketplace post", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
    ...
    @PutMapping("/{id}/edit")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<MarketplacePost>> userEditPost(
            @PathVariable Long id,
            @RequestBody Map<String, Object> updates) {
        try {
            String username = getCurrentUsername();
            MarketplacePost post = marketplaceService.userEditPost(id, updates, username);
            return ResponseUtil.success(post, "Post updated successfully. It will be reviewed again.");
        } catch (Exception e) {
            log.error("Error editing marketplace post", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
