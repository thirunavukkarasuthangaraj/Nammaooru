...
public class FarmerProductController {
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('SUPER_ADMIN')")
    public ResponseEntity<ApiResponse<FarmerProduct>> adminUpdateImages(
            @PathVariable Long id,
        ...
        try {
            List<String> keep = (keepImageUrls != null && !keepImageUrls.isBlank())
                    ...
                    : List.of();
            FarmerProduct post = farmerProductService.adminUpdateImages(id, keep, images);
            return ResponseUtil.success(post, "Images updated successfully");
        } catch (Exception e) {
            log.error("Error updating farmer product images", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
