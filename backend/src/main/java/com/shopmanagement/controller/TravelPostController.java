...
public class TravelPostController {
    ...
    public ResponseEntity<ApiResponse<TravelPost>> toggleFeatured(@PathVariable Long id) {
        try {
            TravelPost post = travelPostService.toggleFeatured(id);
            ...
            log.error("Error toggling featured status for travel post", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
    ...
    @PutMapping("/{id}/renew")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<TravelPost>> renewPost(
            @PathVariable Long id,
            @RequestParam(value = "paidTokenId", required = false) Long paidTokenId) {
        try {
            String username = getCurrentUsername();
            TravelPost post = travelPostService.renewPost(id, paidTokenId, username);
            return ResponseUtil.success(post, "Post renewed successfully");
        } catch (Exception e) {
            log.error("Error renewing travel post", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
    ...
    @PutMapping("/{id}/edit")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<TravelPost>> userEditPost(
            @PathVariable Long id,
            @RequestBody Map<String, Object> updates) {
        try {
            String username = getCurrentUsername();
            TravelPost post = travelPostService.userEditPost(id, updates, username);
            return ResponseUtil.success(post, "Post updated successfully. It will be reviewed again.");
        } catch (Exception e) {
            log.error("Error editing travel post", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
