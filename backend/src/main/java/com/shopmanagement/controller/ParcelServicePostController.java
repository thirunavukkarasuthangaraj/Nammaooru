...
public class ParcelServicePostController {
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('SUPER_ADMIN')")
    public ResponseEntity<ApiResponse<ParcelServicePost>> adminUpdateImages(
            @PathVariable Long id,
        ...
        try {
            List<String> keep = (keepImageUrls != null && !keepImageUrls.isBlank())
                    ...
                    : List.of();
            ParcelServicePost post = parcelServicePostService.adminUpdateImages(id, keep, images);
            return ResponseUtil.success(post, "Images updated successfully");
        } catch (Exception e) {
            log.error("Error updating parcel service post images", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('SUPER_ADMIN')")
    public ResponseEntity<ApiResponse<ParcelServicePost>> toggleFeatured(@PathVariable Long id) {
        try {
            ParcelServicePost post = parcelServicePostService.toggleFeatured(id);
            ...
            log.error("Error toggling featured status for parcel service post", e);
            return ResponseUtil.error(e.getMessage());
        }
    }
