-- Weight variants (1kg, 500g, 250g, 100g) for every vegetable already
-- listed in Murugesan Supermarket (shop_id = 2), reusing the same photo,
-- name, and tags as the original — ONLY base_weight/base_unit differ
-- between variants (the app already shows weight as its own line under
-- the product name, so a distinct name per size would be redundant).
...
-- Step 1: create the 4 weight-variant master products for each vegetable
...
CROSS JOIN (VALUES ('1kg', 1.000, 1.00),
                    ('500g', 0.500, 0.50),
                    ('250g', 0.250, 0.25),
...
WHERE mp.sku LIKE 'VEG-%'
  ...
  -- can't create variants-of-variants (e.g. ...-1KG-1KG)
  AND mp.sku !~ '-(1KG|500G|250G|100G)$'
ON CONFLICT (sku) DO NOTHING;

-- Step 2: copy the same primary image onto each new variant
...
JOIN master_products basemp
   ...
   AND newmp.sku IN (basemp.sku || '-1KG', basemp.sku || '-500G',
                      basemp.sku || '-250G', basemp.sku || '-100G')
