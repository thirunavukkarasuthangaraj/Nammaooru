...
CREATE OR REPLACE FUNCTION update_shop_documents_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_shop_documents_updated_at
    BEFORE UPDATE ON shop_documents
    FOR EACH ROW
    EXECUTE FUNCTION update_shop_documents_updated_at();